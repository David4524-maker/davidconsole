...
# -*- coding: utf-8 -*-
"""
davidconsole KIDS v2.0 🌈
151 comandos multi-idioma (es/en) + lienzo para dibujar.
Interfaz azul (terminal) + rosa (dibujo).
Hecho con ❤️  en Python.
"""

import tkinter as tk
import random, math, time, datetime, re, sys
...
T = {
 "es": {
    "bienvenida": "¡Hola! amigito/a, soy davidconsole, ingresa algo para divertirnos ⭐",
 ...
 "en": {
    "bienvenida": "Hi! friend, I'm davidconsole, type something to have fun ⭐",
...
def dar_estrellas(n=1, extra=""):
    STATE["estrellas"] += n
    print(t("puntos", n=n, total=STATE["estrellas"]))
    ...
    for k, info in INSIGNIAS.items():
        if STATE["estrellas"] >= info["min"] and k not in STATE["insignias"]:
            ...
            print(t("insignia_nueva", nombre=info[STATE["lang"]]))
...
def c(a):
    n = STATE["estrellas"]
    print(f"⭐ Tienes {n} estrellas, {STATE['nombre']}!")
    print("   " + "⭐" * min(n, 30))
...
def c(a):
    ...
    print("   ¡Sigue jugando para subir!")
...
def c(a):
    ...
    print(f"🧮 {' + '.join(map(str,ns))} = {sum(ns)}")
    dar_estrellas(1)
...
def c(a):
    ...
    total = sum(range(1,int(n)+1))
    print(f"✨ 1+2+...+{int(n)} = {total}"); dar_estrellas(2)
...
def c(a):
    ...
    print("   ¡Intenta adivinar qué operación es!")
    dar_estrellas(1)
...
def c(a):
    ...
    s = a[0].lower()
    n = sum(1 for c in s if c in "aeiouáéíóú")
    print(f"🔤 '{s}' tiene ~{n} sílabas"); dar_estrellas(1)
...
def c(a):
    largas = ["esternocleidomastoideo","electroencefalograma","otrora","supercalifragilistico",
...
def c(a):
    h = {"perro":"casa 🏠","gato":"casa 🏠","vaca":"granja 🚜","pato":"estanque 🦆",
         ...
         "delfin":"mar 🌊","mariposa":"jardín 🌸","tortuga":"mar/rio 🐢","abeja":"colmena 🍯",
...
def c(a):
    ideas = ["un 🐶 perro con sombrero","un 🌈 arcoíris brillante","una 🦄 unicornio",
...
def c(a):
    ...
    for i in range(1,11): print(f"   {i}...", end=" ")
...
@comando("dibujo_gato", N("dibujo_gato","draw_cat"), es="Dibuja un gato", en="Draw a cat")
def c(a): _dibujo(r"""
     /\_/\  
    ( o.o ) 
     > ^ <
...
@comando("dibujo_perro", N("dibujo_perro","draw_dog"), es="Dibuja un perro", en="Draw a dog")
def c(a): _dibujo(r"""
    __      _
   /  \    / \
  |    \__/   |
...
@comando("corazon", N("corazon","heart"), es="Dibuja un corazón", en="Draw a heart")
def c(a):
    print("""
   ❤️❤️   ❤️❤️
  ❤️❤️❤️❤️❤️❤️
   ❤️❤️❤️❤️❤️
    ❤️❤️❤️
     ❤️
    """)
    print(f"💖 Para ti, {STATE['nombre']}"); dar_estrellas(1)

@comando("estrella", N("estrella","star"), es="Dibuja una estrella", en="Draw a star")
def c(a):
    print("""
       ⭐
      ⭐⭐
     ⭐⭐⭐
      ⭐⭐
       ⭐
    """)
    print("✨ ¡Brilla como tú!"); dar_estrellas(1)

@comando("bandera", N("bandera","flag"), es="Dibuja una bandera", en="Draw a flag")
def c(a): _dibujo("""
    ┌──────────┐
    │ 🟥🟥🟥🟥 │
    │ 🟨🟨🟨🟨 │
...
@comando("emoji", N("emoji","emoji"), es="Emoji al azar", en="Random emoji")
def c(a):
    print(f"🎲 ¡{random.choice(['😀','🎉','🌈','🚀','🐶','🐱','🦄','🍦','🍭','⭐','💖','🌸','🎈','🎨','🍕'])}!")
    dar_estrellas(1)

@comando("carita", N("carita","face"), es="Carita feliz", en="Happy face")
def c(a):
    print(f"¡{random.choice(['😀','😊','😄','🤗','😎','🥳','🤩','😇'])} para {STATE['nombre']}!")
    dar_estrellas(1)

@comando("banner_kids", N("banner_kids","kids_banner"), es="Banner: banner_kids Ana", en="Banner")
def c(a):
    txt = " ".join(a) if a else STATE["nombre"]
    print("🌟" * (len(txt)+4)); print(f"🌟 {txt.upper()} 🌟"); print("🌟" * (len(txt)+4))
    dar_estrellas(1)

@comando("dibujo_sol", N("dibujo_sol","draw_sun"), es="Dibuja el sol", en="Draw a sun")
def c(a): _dibujo(r"""
      \ | /
    -- ☀️ --
      / | \
...
@comando("dibujo_luna", N("dibujo_luna","draw_moon"), es="Dibuja la luna", en="Draw a moon")
def c(a): _dibujo("""
    🌙  *  ✨
   *  *  *
""", "🌙", "¡La luna llena!") (a)

@comando("dibujo_casa", N("dibujo_casa","draw_house"), es="Dibuja una casa", en="Draw a house")
def c(a): _dibujo(r"""
      /\
     /  \
    /____\
...
@comando("dibujo_arbol", N("dibujo_arbol","draw_tree"), es="Dibuja un árbol", en="Draw a tree")
def c(a): _dibujo(r"""
       🌳
      /|\
     / | \
...
@comando("dibujo_flor", N("dibujo_flor","draw_flower"), es="Dibuja una flor", en="Draw a flower")
def c(a): _dibujo(r"""
      🌸
    🌸🌸🌸
      🌸
...
@comando("dibujo_helado", N("dibujo_helado","draw_ice_cream"), es="Dibuja un helado", en="Draw ice cream")
def c(a): _dibujo(r"""
     🍦
    ( )
     |
...
@comando("dibujo_coche", N("dibujo_coche","draw_car"), es="Dibuja un coche", en="Draw a car")
def c(a): _dibujo(r"""
      ____
     /    \
    |  []  |
...
CATEGORIAS = {
 ...
 "✏️ Arte": ["dibujo_gato","dibujo_perro","corazon","estrella","bandera","emoji","carita","banner_kids",
             "dibujo_sol","dibujo_luna","dibujo_casa","dibujo_arbol","dibujo_flor","dibujo_helado","dibujo_coche"],
...
def c(a):
    print("🎨 davidconsole KIDS v2.0 — Hecho con ❤️  en Python")
    print(f"   ¡Gracias por jugar, {STATE['nombre']}! 🌟")

build_index()

# ============================================================
# INTERFAZ GRÁFICA (azul + rosa)
# ============================================================
class App:
    def __init__(self, root):
        self.root = root
        root.title("davidconsole KIDS 🌈")
        root.geometry("1400x700")
        root.minsize(1000, 500)
        root.configure(bg=C_BLUE)

        # ------- CONTENEDOR PRINCIPAL -------
        main = tk.Frame(root, bg=C_BLUE)
        main.pack(fill="both", expand=True)

        # ============ PANEL IZQUIERDO (azul, terminal) ============
        left = tk.Frame(main, bg=C_BLUE)
        left.pack(side="left", fill="both", expand=True)

        # Contador de estrellas (arriba izquierda)
        self.lbl_estrellas = tk.Label(left, text="⭐ 0", bg=C_BLUE, fg=C_YELLOW,
                                       font=("Arial", 22, "bold"))
        self.lbl_estrellas.pack(anchor="nw", padx=15, pady=(10, 0))

        # Área de texto verde
        self.text = tk.Text(left, bg=C_BLUE, fg=C_GREEN, font=("Consolas", 14, "bold"),
                            wrap="word", insertbackground=C_WHITE, bd=0,
                            padx=15, pady=5, selectbackground="#0066CC",
                            highlightthickness=0)
        self.text.pack(fill="both", expand=True, padx=15, pady=(5, 15))

        self.text.tag_configure("green",  foreground=C_GREEN)
        self.text.tag_configure("white",  foreground=C_WHITE)
        self.text.tag_configure("yellow", foreground=C_YELLOW)
        self.text.tag_configure("cyan",   foreground="#00FFFF")
        self.text.tag_configure("gray",   foreground="#CCCCCC")
        ...
        # ============ PANEL DERECHO (rosa, dibujo) ============
        right = tk.Frame(main, bg=C_PINK, width=650)
        right.pack(side="right", fill="both", expand=False)
        right.pack_propagate(False)

        # Logo arriba a la derecha
        logo_top = tk.Frame(right, bg=C_PINK)
        logo_top.pack(anchor="ne", padx=20, pady=(15, 5))

        logo = tk.Canvas(logo_top, width=280, height=110, bg=C_PINK, highlightthickness=0)
        logo.pack()
        # Borde claro
        logo.create_rectangle(2, 2, 278, 108, outline=C_LGREY, width=5)
        # Caja gris
        logo.create_rectangle(12, 12, 268, 98, fill="#808080", outline="#555")
        # Flecha blanca
        logo.create_text(45, 55, text=">", fill="white", font=("Arial", 45, "bold"))
        # david (naranja) y kids (verde)
        logo.create_text(130, 42, text="david", fill=C_ORANGE, font=("Arial", 18, "bold"))
        logo.create_text(130, 72, text="kids", fill=C_GREEN, font=("Arial", 18, "bold"))
        # Cara sonriente
        logo.create_oval(200, 30, 250, 80, outline="black", width=3)
        logo.create_oval(212, 42, 222, 54, fill="black")
        logo.create_oval(228, 42, 238, 54, fill="black")
        logo.create_arc(210, 45, 240, 72, start=200, extent=140, style="arc", outline="black", width=3)

        # Título "Dibuja tu protada"
        tk.Label(right, text="Dibuja tu portada", bg=C_PINK, fg=C_GRAY,
                 font=("Arial", 20, "bold")).pack(anchor="w", padx=30, pady=(45, 10))

        # Lienzo blanco para dibujar
        self.canvas = tk.Canvas(right, bg="white", bd=0, highlightthickness=0)
        self.canvas.pack(fill="both", expand=True, padx=30, pady=(0, 30))
        self.canvas.bind("<B1-Motion>", self.dibujar)
        self.canvas.bind("<Button-1>", self.iniciar_dibujo)
        self.canvas.bind("<Button-3>", self.limpiar_lienzo)
    ...
    def dibujar(self, e):
        if self.last_x is not None:
            self.canvas.create_line(self.last_x, self.last_y, e.x, e.y,
                                     fill="#333333", width=3, capstyle="round", smooth=True)
        self.last_x, self.last_y = e.x, e.y

    def iniciar_dibujo(self, e):
        self.last_x, self.last_y = e.x, e.y

    def limpiar_lienzo(self, e=None):
        self.canvas.delete("all")

    def write(self, s):
        if s: self._w(s, "green")
    def flush(self): pass
    ...
    def _w(self, s, tag):
        ...
        self.lbl_estrellas.config(text=f"⭐ {STATE['estrellas']}")

    def new_prompt(self):
        self.text.insert("end", f"[{STATE['nombre']}]  ", "white")
        self.text.insert("end", f"[{STATE['lang']}] ", "cyan")
    ...
    def on_key(self, event):
        if event.keysym in ("Shift_L","Shift_R","Control_L","Control_R","Alt_L","Alt_R",
                            "Caps_Lock","Escape","Tab","Return","KP_Enter"): return
    ...
    def on_enter(self, event):
        cmd = self.text.get(self.input_start_index, "end-1c").strip()
    ...
    def execute(self, linea):
        ...
        if canonical is None:
            print(t("desconocido", cmd=nombre)); return
        try:
